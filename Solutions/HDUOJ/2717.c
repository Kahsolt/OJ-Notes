  ...
  if (s < t) {
    ...
    for (int i=0; i<ans; i++)
      for (int j=0; j<=MAXN; j++) {
        int nextj;
        nextj = j + 1; if (nextj <= MAXN && axis[j] + 1 < axis[nextj]) axis[nextj] = axis[j] + 1;
        nextj = j - 1; if (nextj >= 0    && axis[j] + 1 < axis[nextj]) axis[nextj] = axis[j] + 1;
        nextj = j * 2; if (nextj <= MAXN && axis[j] + 1 < axis[nextj]) axis[nextj] = axis[j] + 1;
      }
    ans = axis[t];
  }
  return ans;
